...
exports.updateProduct = async (details) => {
    try {
        const collecton = dbConnect.db.collection('products');
        const product = collecton.findOneAndUpdate({ product_id: details.product_id }, { $set: { ...details } }, { "returnNewDocument": true })
        return product;
    } catch (error) {
        return { "error": error };
    }
